...
"""
import base64, hashlib, hmac, http.server, json, os, random, re, secrets, socket, socketserver, threading, time, urllib.parse, sys
from datetime import datetime

PORT = int(sys.argv[1]) if len(sys.argv) > 1 else 9191
ADMIN_PIN_SALT = b'KoreaGang.admin.v2'
ADMIN_PIN_ITERATIONS = 240000
ADMIN_PIN_HASH_HEX = '8f193489ac9f97ba9ef81fc0763dc60ee3adee92cbc268e8c993e001b6c4ea39'
...
def _pin_digits(pin):
    ...
    if not re.fullmatch(r'\d{6}', pin):
        return None
    return [int(ch) for ch in pin]

def _pin_fold(digits):
    acc = 0
    for idx, digit in enumerate(digits):
        acc = acc * 13 + digit + idx
    return acc

def _pin_shape_ok(pin):
    digits = _pin_digits(pin)
    if not digits:
        return False
    total = 0
    product = 1
    for digit in digits:
        total += digit
        product *= digit
    return total == 36 and product == 38416 and _pin_fold(digits) == 2842563

def _derive_admin_pin_hex(pin):
...
def admin_pin_ok(pin):
    pin = str(pin or '').strip()
    if not _pin_shape_ok(pin):
        return False
    return hmac.compare_digest(_derive_admin_pin_hex(pin), ADMIN_PIN_HASH_HEX)

def _purge_admin_tokens(now=None):
    now = time.time() if now is None else now
    expired = [token for token, expiry in ADMIN_TOKENS.items() if expiry <= now]
...
def issue_admin_token():
    now = time.time()
    with _admin_lock:
        _purge_admin_tokens(now)
        token = secrets.token_urlsafe(32)
        ADMIN_TOKENS[token] = now + ADMIN_SESSION_TTL_SECONDS
        return token

def admin_token_ok(token):
    token = str(token or '')
    if not token:
        return False
    now = time.time()
...
class H(http.server.SimpleHTTPRequestHandler):
    ...
    def api_post(self):
        ...
        log_data = {k: v for k, v in body.items() if k not in {'image', 'pin', 'token'}}
        ...
        if route == '/api/admin/auth':
            if admin_pin_ok(body.get('pin', '')):
                return self.ok({'ok': True, 'token': issue_admin_token()})
            return self.ok({'ok': False})
        ...
        with _lock:
            ...
            if route == '/api/admin/set_day_hide':
                if not admin_token_ok(body.get('token', '')): self.send_error(403); return
